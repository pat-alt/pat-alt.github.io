...
The problem was (I believe) that the `generator` variable existed in the global scope (`Main`) but it was not accessible for the `@objective` macro that at runtime lives in (`Main.Generators`). Fortunately, it is easy to make the variable accessible by wrapping it inside and `esc()` call:
...
In this blog post, I've done something I usually try to avoid: talk about stuff I know next to nothing about. Metaprogramming is an exciting topic and if you're still here, you just got to experience it through the lens of an absolute novice. During our leap of faith into Julia's metaverse we've learned the following things:
...
Throughout this post, I have skipped various important details that (I think) were not immediately relevant to the goal I had in mind: adding my first macro to  [`CounterfactualExplanations.jl`](https://github.com/JuliaTrustworthyAI/CounterfactualExplanations.jl). 
