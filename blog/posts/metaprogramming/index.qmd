...
Finally, let's also define a simple macro that can be used to change the decision threshold $\gamma$ that is used to specify when the counterfactual search is considered to have converged:

```{julia}
macro threshold(generator, γ)
    return :(generator.decision_threshold=$γ; generator)
end
@threshold(generator, 0.75)
```

In this case, using a macro is most definitely not necessary, but this should help to keep the syntax for composing generators consistent.
