...
The goal is to use a macro that builds an expression that, when evaluated, instantiates a composable generator. In particular, the macro should accept as its main argument an expression that looks much like the right-hand-side of @eq-general, which is essentially just a weighted sum. 
...
macro objective(ex)
    ...
    ex_generator = :(ComposableGenerator(loss, cost, λ))
    return ex_generator
end
```

I forgot to interpolate ...
