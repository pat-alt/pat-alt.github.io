...
# Results
regtable(ols_all_latent, ols_best_latent, ols_ar, ols_factors, ols_best; renderSettings = asciiOutput())
```
...
```{julia}
df_growth = @select(df_gdp, Date:growth) |>
    x -> @mutate(x, yl=lag(growth))
df_daily = @full_join(df_growth, @mutate(df_wide, Date=Date(Date)), Date) |>
    x -> @arrange(x, Date) |>
    x -> @filter(x, Date <= Date("2018-12-31")) |>
    x -> @fill_missing(x, growth, yl, "down") |>
    dropmissing
yields = df_daily |>
    x -> @select(x, -(Date:yl)) |> 
    Matrix |>
    x -> Float32.(x) 
A = encoder(yields')'[:,[3,5]]
yl = df_daily.yl
X = hcat(ones(length(yl)),yl, A)
y = df_daily.growth
yhat = predict(ols_best, X)
plt_gdp = plot(
    df_daily.Date, y;
    label="Actual", color=:green,
    title="Modelling GDP Growth",
...
yhat_full = model(yields')'
plot!(
    df_daily.Date,yhat_full; 
    label="Predicted (full)", color=:orange
)
```
