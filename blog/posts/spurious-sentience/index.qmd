...
```{julia}
reg_data = DataFrame(hcat(y, A),["y",["a$i" for i in 1:size(A,2)]...])
lm(@formula(y ~ a1 + a2 + a3 + a4 + a5 + a6), reg_data)
```
...
```{julia}
yl = hcat(lag(y),A)[2:end,:]
ϕ = (yl'yl) \ yl'y[2:end]
ŷ = yl * ϕ

plt_qtr = plot(y[2:end])
plot!(ŷ)
```
...
```{julia}
# Daily yields:
date_min, date_max = extrema(df_all.Date)
df_daily = df_wide[Date.(df_wide.Date) .>= date_min .&& Date.(df_wide.Date) .<= date_max, :]
X = df_daily |>
    x -> @select(x, -Date) |> Matrix
A = encoder(X')' 
ŷ = A * β
plt_daily = plot(ŷ)
```
