...
time = Observable(minimum(df.year))
plt_data = @lift((
  mat = df.variable[df.year.==$time],
  val = df.value[df.year.==$time],
  date = df.Date[df.year.==$time],
))
# Animation:
fr = 30
timestamps = sort(unique(df.year))
plt = data(plt_data) * 
    mapping(
      :mat => "Maturity (years)", 
      :value => "Yield (%)", 
    ...
    visual(Lines, color=:blue, alpha=0.1)
fig = draw(plt)
file_path = joinpath(BLOG_DIR, "www/yields.gif")
record(fig.figure, file_path, timestamps; framerate = fr) do t
    time[] = t  
end
```
