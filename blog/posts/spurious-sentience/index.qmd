...
tfm = load_model(model_name; config = cfg)
```

```{julia}
# Encode:
n = 100
train_data = data |> shuffle |> df -> df[1:n,:] 
y = train_data.label |> 
    y -> Flux.OneHotArrays.onehotbatch(y, [0,1,2]) 
query = train_data.sentence
a = encode(tkr, query)
b = tfm(a)
```
...
k = n_layers - 1
@assert k > 0 && k <= n_layers
A = b.outputs[k].hidden_state 
dl = Flux.DataLoader((A, y), batchsize=10) 
n_in = size(A,1)
n_out = length(labels)
model = Flux.Chain(
    Transformers.HuggingFace.FirstTokenPooler(),
    Dense(n_in, n_out)
)
loss(y_hat, y) = Flux.logitcrossentropy(y_hat, y)
rule = Flux.Adam()
opt_state = Flux.setup(OptimiserChain(WeightDecay(λ), rule), model) # weight decay is L2 regularization
```
...
```{julia}
sum(Flux.onecold(softmax(model(A)), [0,1,2]) .== train_data.label)/n
```

```{julia}
query = [
    "This raised the hawks' interest in prey.",
    "This raised the hawks' interest by 10 basis points.",
    "This raised the hawks' interest by 50 basis points.",
    "This raised the hawks' interest by 100 basis points.",
    "This lowered the hawks' interest by 10 basis points.",
    "This lowered the hawks' interest by 50 basis points.",
    "This lowered the hawks' interest by 100 basis points.",
]
a = encode(tkr, query)
b = tfm(a)
_A = b.hidden_state
...
res = DataFrame(query=query,predicted=yhat,probability=vec(maximum(d,dims=1)))
```
