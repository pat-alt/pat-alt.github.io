...
# Utility functions:
include(joinpath(BLOG_DIR, "utils.jl"))
```
...
@fig-pca shows the first two principal components of the yield curves of US Treasury bonds over time. For comparison, @fig-obs shows the average level and spread of the yield curves over time. The first principal component is strongly correlated with the level of the yield curve, while the second principal component is strongly correlated with the spread of the yield curve. To put it in AI-lingo:

> The estimated latent embeddings of the yield curve are characterized by patterns observed in the data. 

```{julia}
#| fig-cap: First two principal components of US Treasury bonds.
#| label: fig-pca
#| output: true
...
```{julia}
#| fig-cap: Average level and spread of US Treasury bonds.
#| label: fig-obs
#| output: true
...
Not convinced? Let us use $\mathbf{Y}=\mathbf{U}\Sigma\mathbf{V}^{\prime}$ in true autoencoder fashion to reconstruct yield curves from principal components. 
...
### Yield Curves and "Deep Learning"
