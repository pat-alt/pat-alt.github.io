...
tags: [bayes, logit, julialang]
subtitle: From scratch in Julia Language
summary: 'An introduction to Bayesian Logistic Regression from the bottom up with examples in Julia language.'
authors: []
...
```{r ground, fig.width=5, fig.height=5, fig.cap="Ground truth labels."}
dt_plot <- copy(dt) 
...
```{r probit, fig.cap="Demonstration of the probit approximation."}
a <- seq(-2,2,0.01)
p_true <- 1/(1+exp(-a))
p_norm <- pnorm(a)
...
```{r posterior, fig.cap="Posterior distribution for $w_2$ and $w_3$ at varying degrees of prior uncertainty $\\sigma$."}
...
```{r predictive, fig.cap="Predictive posterior distribution at varying degrees of prior uncertainty $\\sigma$."}
