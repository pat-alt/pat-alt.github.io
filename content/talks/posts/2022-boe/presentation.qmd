...
## Counterfactual Explanations vs. Feature Attribution Methods
...
counterfactual_data = data_sets[:credit_default]
...
using Tables
tab_input = Tables.table(permutedims(data.X), header=input_names)
using DataFrames
...
target_class = 0 
non_target_class = 1
x = select_factual(counterfactual_data,rand(findall(vec(ys).==non_target_class)))
```
...
# Questions & Answers ❓ {background-color="#ffffff" background-size="30%" background-position="98% 98%" background-image="/www/images/boe.svg"}
