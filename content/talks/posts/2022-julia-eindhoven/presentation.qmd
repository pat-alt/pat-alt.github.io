...
> A plain Markdown document blended with **your favorite programming language** of your choice and a YAML header defining your output.
...
> Quarto enables enables effective communication and reproducibility without compromises. 
...
- The slides you are looking at are not only built using Quarto, but also hosted on a [website](https://www.paltmeyer.com/) that is also run on Quarto.
 
. . .

- Quarto uses something call listing pages, to make it easy to collect and navigate content like this one. 
...
- You can specify YAML options such that changes to your underlying Julia code will trigger your blog post to be rerendered. This essentially allows you to easily test that the code you publish actually runs:
...
- The Julia environment can be managed globally or locally for individual blog posts:
...
- The use of `jldoctest` is not always straight-forward (see [here](https://github.com/JuliaDocs/Documenter.jl/issues/452)). Letting docs run through the Quarto engine provides an additional layer quality assurance.
