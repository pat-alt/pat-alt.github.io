...
#let poster(
  ...
  // Optional full-width image to display below the header (path or none).
  tldr_image: none,

  // Optional TLDR text to display above the image.
  tldr: none,

  // Font size for the TLDR text (in pt).
  tldr_font_size: "28",

  // Vertical space after the TLDR image (in pt).
  tldr_image_vspace: "20",
  ...
  // TLDR:
  tldr_font_size = int(tldr_font_size) * 1pt
  tldr_image_vspace = int(tldr_image_vspace) * 1pt

  if tldr != none or tldr_image != none {
    v(tldr_image_vspace)
    if tldr != none {
      block(
        fill: rgb("D9F0F6"),
        ...
        radius: 4pt,
        width: 100%,
        text(size: tldr_font_size, tldr)
      )
      v(10pt)
    }
    if tldr_image != none {
      image(tldr_image, width: 100%)
    }
    v(tldr_image_vspace)
  }
